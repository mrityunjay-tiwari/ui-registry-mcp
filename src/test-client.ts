...
async function main() {
  ...
  check("returns >= 12 registries", Array.isArray(regs) && regs.length >= 12, `count=${regs.length}`);
  check("includes all newly added registries", ["reui", "kibo", "aceternity", "tailark", "cult-ui", "optics", "uselayouts", "watermelon", "beui", "ai-elements"].every((id) => regs.some((r: any) => r.id === id)));
